...
struct ThreadwiseGenericTensorSliceCopy_v4r2
{
    ...
    __device__ void Run(const SrcData* p_src, DstData* p_dst,bool bPrint = false) const
    {
        ...
        ford<decltype(long_vector_access_lengths), SrcDstDimAccessOrder>{}([&](
            ...
            for(index_t i = 0; i < long_vector_size / dst_data_per_access; ++i)
            {
                ...
                if(bPrint)
                {
                    printf("\n block id:%d,threadid:%d , p_dst: %p  dst_coord:%d , p_dst_long_vector:%p buffer_offset: %d, dst_data_per_access:%d",
